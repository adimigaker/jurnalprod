...
function renderBeranda() {
    ...
    pg.innerHTML =
        `<div class="section-header"><span class="section-title">Hari Ini</span><span style="font-size:12px;color:var(--text-muted);font-family:var(--mono)">${fmtShort(today())}</span></div>` +
...
function renderSummary(p) {
    ...
    const diffLabel = (diff, target) => {
        ...
        if (diff < 0)
            return {
                text: `Kurang dari target (${target})`,
                cls: "diff-negative"
            };
